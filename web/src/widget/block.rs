...
/// A container that distributes its contents horizontally or vertically.
///
/// A [`Block`] will try to fill the horizontal space of its container.
...
impl<'a, Message> Block<'a, Message> {
    ...
    /// Sets the vertical alignment of the contents of the [`Block`] .
...
where
    Message: 'static,
{
    fn from(column: Block<'a, Message>) -> Element<'a, Message> {
        Element::new(column)
    }
}
